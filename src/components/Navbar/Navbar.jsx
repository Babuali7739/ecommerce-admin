...
const Navbar = () => {
  ...
  const handleSearch = async (e) => {
    ...
    if (query) {
      try {
        const response = await axios.get(`https://ecommerce-backend-9dq4.onrender.com/search?q=${query}`);
  ...
  const performSearch = async () => {
    if (searchQuery) {
      try {
        const response = await axios.get(`https://ecommerce-backend-9dq4.onrender.com/search?q=${searchQuery}`);
